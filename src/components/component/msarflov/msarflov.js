import {  Button, FormControl, InputLabel, MenuItem, Paper, Select, Snackbar, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextareaAutosize, TextField } from '@mui/material';
import { Link, useParams } from 'react-router-dom';
import * as React from 'react';
import ArrowBackIcon from "@mui/icons-material/ArrowBack";
import './msarflov.scss'
...
const findName = arr.find(el => el.name === per)
   ...
   function addFile(value) {
    ...
    // console.log(down);
    console.log('person?.mahsulotlar',person);
      ...
      <div className="sarflov msarflov"  >
        <div className="sarflov_inner">
          <div className="sarflov_inner_left">
            ...
            <Button onClick={() => pdfExportComponent.current.save()} style={{backgroundColor:"#E63943",color:"#fff"}} startIcon={<DescriptionIcon />} variant="contained">Pdf shaklida yuklash</Button>
            </div>
          </div>

         {
           person.status === `Ariza jo'natildi` ?  <div className="status_btn_block">
            <Button variant="contained"  onClick={(e) => Statusall(person.ariza_id, e.target.value)} value='rad qilish' startIcon={<CloseIcon/>}>Rad qilish</Button>
            <Button variant="contained"  onClick={(e) => Statusall(person.ariza_id, e.target.value)} value='qabul qilish' startIcon={<CheckIcon/>}>Qabul qilish</Button>
          </div> : person.status === 'korildi' ?
           <div className="status_btn_block">
            <Button variant="contained"  onClick={(e) => Statusall(person.ariza_id, e.target.value)} value='rad qilish' startIcon={<CloseIcon/>}>Rad qilish</Button>
            <Button variant="contained"  onClick={(e) => Statusall(person.ariza_id, e.target.value)} value='qabul qilish' startIcon={<CheckIcon/>}>Qabul qilish</Button>
          </div> :
          <div className="status_btn_block">
            <Button variant="contained" disabled onClick={(e) => Statusall(person.ariza_id, e.target.value)} value='rad qilish' startIcon={<CloseIcon/>}>Rad qilish</Button>
            <Button variant="contained" disabled onClick={(e) => Statusall(person.ariza_id, e.target.value)} value='qabul qilish' startIcon={<CheckIcon/>}>Qabul qilish</Button>
          </div>
         }
        </div>
        ...
        <div className="sarflov_block" >
          <h4 className="sarflov_block_title">Kimdan va kimga</h4>
          <div className="sarflov_block_inner">
            <div className="sarflov_block_inner_div">
              <h5 className="sarflov_block_inner_div_title">
                Kimdan (Direktor ism sharifi)
              </h5>
              {/* <FormControl fullWidth>
                <InputLabel id="demo-simple-select1">H.G. Ganiyev</InputLabel>
                <Select
                ...
                >
                  <MenuItem value="H.G. Ganiyev">H.G. Ganiyev</MenuItem>
                </Select>
              </FormControl> */}
            ...
            <div className="sarflov_block_inner_div">
              <h5 className="sarflov_block_inner_div_title">Kimga</h5>
              {/* <TextField
                id="outlined-basic"
                label="Minzdravga"
                variant="outlined"
        ...
        <div div className="sarflov_block sarflov_block_m">
          <h4 className="sarflov_block_title">{person.ariza_turi === "sarflov"?"Vositalar":"Jihozlar"}</h4>
                ...
                <TableHead>
                  <TableRow style={{ backgroundColor: "white" }}>
                    ...
                    <TableCell
                      ...
                      align="left"
                    >
                     {person.ariza_turi === "sarflov"?"Vosita nomi":"Jihoz nomi"} 
                    </TableCell>
                    <TableCell
                      style={{
                        fontWeight: "bold",
                      ...
                      align="left"
                    >
                      {person.ariza_turi === "sarflov" ?"O’ lchov birligi":"Jihoz Ma'lumoti"}
                    </TableCell>
                    <TableCell
                      style={{
                        fontWeight: "bold",
                  ...
                  {person?.mahsulotlar
                    ...
                    .map((item, index) => (
                      <TableRow>
                        ...
                        <TableCell align="left">{item.mahsulot_nomi === "Igna" && "Igna Arterial"} {item.mahsulot_nomi === "Igna2" && "Igna Venoz"} {!item.mahsulot_nomi.includes("Igna") && item.mahsulot_nomi}</TableCell>
                        <TableCell align="left">
                          {person.ariza_turi === "sarflov" ?item.mahsulot_olchov_birligi:item.mahsulot_turi}
                        </TableCell>
                        <TableCell align="left">{item.miqdori}</TableCell>
                        <TableCell align="left">{person.ariza_turi === "sarflov" && findName.code}</TableCell>
                      </TableRow>
                    ))}
                </TableBody>
              </Table>
        ...
        <div className="sarflov_comment">
          <div className="sarflov_block_comment">
            <h4 className="sarflov_block_title">Qo’shimcha ma’lumot</h4>
          ...
          <div className="sarflov_block_comment">
            <div className="sarflov_block_comment_inner">
              <h4 className="sarflov_block_title">Yuborilgan hujjatlar</h4>
              {
                person.fayllar.length  ? <a
                className='download_to_file'
                  target="_blank"
                  href={`https://admin-nefro.ssv.uz/static/${person?.fayllar[0].file}`}
                >
                  yuklash
                </a> : null
              }
        ...
        <Stack spacing={2} sx={{ width: '100%' }}>
            <Snackbar open={open} autoHideDuration={6000} onClose={handleClose} style={{
              textTransform:"uppercase"
            }} >
              <Alert  onClose={handleClose} severity={statuslar === "rad qilish"? "error":"success"}>{statuslar} !</Alert>
            </Snackbar>
          </Stack>
      </div>
    );
